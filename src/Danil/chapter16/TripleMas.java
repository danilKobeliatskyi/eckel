...
public class TripleMas {
    static double[][][] create3Array(int size1, int size2, int size3, double start, double end) {
        if(!(start < end)) {
        ...
        for(int i = 0; i < size1; i++)
            result[i] = DoubleMas.createArray(size2, size3, start, end);
        return result;
    }
